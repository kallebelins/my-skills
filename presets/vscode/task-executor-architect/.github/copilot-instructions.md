...
## Execution → `task-executor-architect`

Triggers: implement a task, mark done, execute an item from `tasks.md` / backlog.

1. Read and follow the installed `task-executor-architect` skill before acting.
2. Select and follow an architect skill from loaded skills when the task involves structure, patterns, or trade-offs.
...
This project expects the `spec-kit` and `task-executor-architect` skills from [kallebelins/my-skills](https://github.com/kallebelins/my-skills) to be installed and available to the agent.
